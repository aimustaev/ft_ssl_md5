...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/01/09 20:26:01 by aimustaev         #+#    #+#             */
...
static char				*sha384tostr(uint64_t hashbuff[8])
{
	char	*out;
	int		i;
	uint8_t	*p;
	...
	while (i < 6)
	{
		...
		while (j < 8)
		{
			out[i * 16 + j * 2] = char_to_hex((p[7 - j] >> 4));
			out[i * 16 + j * 2 + 1] = char_to_hex(p[7 - j]);
			j++;
		}
		i++;
	}
...
/*
** SHA512 and SHA384 use the same processing algorithm,
** only differing init_hash and to string.
*/
